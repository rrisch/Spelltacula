...
const props = defineProps({
  ...
  showStart: {type: Boolean, required: false}
});
...
<template>
  <ul class="list bg-base-100 rounded-box shadow-md">
    <li class="p-4 pb-2 text-2xl opacity-60 tracking-wide">{{ title ? title : 'Test Templates' }}</li>
    <li :title="`Start ${test.title}`" v-on:mouseenter="onHover(true,test)" v-on:mouseleave="onHover(false,test)"
      ...
      <div v-show="currentItem != test || !props.showStart" class="text-4xl font-thin opacity-30 tabular-nums">
        {{ index < 9 ? `0${index + 1}` : index + 1 }}
      </div>
      <div v-show="currentItem == test && props.showStart" class="text-4xl font-thin tabular-nums">
      ...
      <button :title="`Edit ${test.title}`" @click.prevent.stop="onTemplateEditClick(test)"
              v-show="currentItem == test && props.showEdit"
